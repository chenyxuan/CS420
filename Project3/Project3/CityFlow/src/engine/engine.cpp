        ...
        for (Drivable *drivable : drivables) {
            while (!drivable->getVehicles().empty()) {
                ...
                if (vehicle->hasSetEnd()) {
                    ...
                    auto it = vehicleStartTime.find(vehicle->getPriority());
                    threadVehiclePool[iter->second.second].erase(vehicle);
                    delete vehicle;
                    iter = vehiclePool.erase(iter);
                    travelTime += getCurrentTime() - it->second;
                    vehicleStartTime.erase(it);
                    activeVehicleCount--;
                } else if (!check)
                    break;
            }
...
//            auto &vehicle = vehicle_pair.second.first;
    ...
    void Engine::pushVehicle(Vehicle *const vehicle) {
        ...
        vehicleStartTime.emplace(vehicle->getPriority(), getCurrentTime());
    ...
    void Engine::reset() {
        ...
        vehicleStartTime.clear();
        vehiclePool.clear();
        roadnet.reset();
        logOut.close();
        logOut.open(this->logFile);
    ...
    double Engine::getAverageTravelTime() const {
        double totalTime = this->travelTime;
        for (auto &vehicleTimePair : vehicleStartTime)
            totalTime += getCurrentTime() - vehicleTimePair.second;
        return totalTime / totalVehicleNum;
    }
